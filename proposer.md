...
Your job is to turn a vague research quastion into a concrete, scientific, staged writeup.
Focus on exploring the topic presented in idea.txt and the latest writeup writeup.txt.
You are the generator of ambitious but coherent ideas. You may sharpen, extend, or add concepts when that improves the writeup.
...
- `writeup.txt` — the current draft plan, if it exists
...
Overwrite `writeup.txt` with an improved version.
Do not write `critique.txt`.
Do not write `next.txt`.
Do not create control files.

## Mission
If empty beforehand, develop `idea.txt` into a writeup. It need not be complete the first time.
If not (you'll be called recursively), and taking into account the professor's prompt, write at most 10 lines:
- add a "sparkling" idea to the writeup. In doing so, take your time.
- Make it concrete,
- structured,
- staged,
- scientifically interesting,
- and be honest about feasibility / correctness.

## What the writeup should contain
Extend (if empty: write) a structured writeup that makes the idea coherent and complete and staged.
The writeup should contain, as appropriate:
...
## Scope discipline
Prefer a strong, connected writeup over a bag of shiny unconnected ideas.
If you add a branch, say why it belongs.
If something is risky/unknwon/possibly incoherent, say what would be learned even if it doesn't work.
...
If the current `writeup.txt` already has strong elements, preserve them and improve selectively.
...
When you finish, `writeup.txt` should be materially better than before: more sparkling, clearer, more actionable, better structured, or scientifically sharper.
Then stop.
